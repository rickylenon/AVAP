	...
	public class LogMethods //: System.IDisposable
	{
		...
		public void SetLogPath()
		{
			...
			if (m_logbasepath == string.Empty)
				sFilePath = System.Environment.CurrentDirectory.TrimEnd('\\') + "\\" + sLogName + m_logfiletype;
			else
				sFilePath = m_logbasepath.TrimEnd('\\') + @"\" + sLogName + m_logfiletype;
			...
			if (m_loglevel >= loglevel)
			{
				...
				switch (logprefix)
				{
					...
					case elogprefix.dt_loglevel:
						logtext = dt.ToString("yyyy.MM.dd") + "-" + dt.ToString("hh.mm.ss") + ":" + loglevel.ToString().ToUpper() + ": " + logtext;
						break;
				}
