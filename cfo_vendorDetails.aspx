<%@ Page Title="" Language="C#" MasterPageFile="~/MasterSubPage.master" AutoEventWireup="true" CodeFile="cfo_vendorDetails.aspx.cs" Inherits="cfo_vendorDetails" %>
...
<%@ Register TagPrefix="Ava" TagName="TopNav" Src="usercontrols/TopNav_faafinance.ascx" %>
...
<div class="separator1"></div>
        <table style="width: 100%;">
            <tr>
                ...
                <td valign="top" width="50%">
                <h3 style="margin:10px 0px;">Overall Evaluation</h3>
                ...
                <br /><asp:Label runat="server" ID="recodate" ></asp:Label><br /><br />
                    ...
                    <div class="clearfix"></div>
                <br />
                    <strong>This is a recommendation to <asp:Label ID="recommendation" runat="server" Text="______" ForeColor="Blue"></asp:Label> the accreditation.</strong><br />
                    <div class="clearfix"><br /><br /></div>
        <div style="float:left; width:30px;"><img alt="" src="images/attachment.png" /></div> 
...
<div class="separator1"></div>
        ...
        <table style="width: 100%;">
            <tr>
                ...
                <br />
                <asp:Repeater ID="repeatervmRecoComment" runat="server" DataSourceID="dsvmRecoComment">
  <ItemTemplate>
    <p><strong><%# Eval("Firstname")%> <%# Eval("Lastname")%></strong>&nbsp;&nbsp;&nbsp;<em><%# Eval("DateCreated")%></em></p>
    <p><%# Eval("Comment").ToString().Replace("\n","<br>")%><br /><br />
  </ItemTemplate>
</asp:Repeater>
  <asp:SqlDataSource ID="dsvmRecoComment" runat="server" ConnectionString="<%$ ConnectionStrings:AVAConnectionString %>"
            SelectCommand="select * from (select t1.Comment, t1.DateCreated, t2.FirstName, t2.Lastname, t3.UserType from tblComments t1, tblUsers t2, tblUserTypes t3 WHERE t2.UserId = t1.UserId AND t1.VendorId = @VendorId AND t3.UserId = t1.UserId) t0 ORDER BY DateCreated DESC" >
    <SelectParameters>
        <asp:SessionParameter Name="VendorId" SessionField="VendorId" Type="Int32" />
                ...
                <td valign="top" width="50%">
                    ...
                    <asp:Label runat="server" ID="recoby2" Font-Bold="true" ></asp:Label><br />
                    <asp:Label runat="server" ID="recodate2" ></asp:Label><br />
                    <strong>This is an endorsement to <asp:Label ID="recommendation2" runat="server" Text="______" ForeColor="Blue"></asp:Label> the accreditation.</strong><br />
                    
                <div class="separator1"></div>
                <h3 style="margin:10px 0px;">CFO COMMENT: </h3>
                <br />
...
<div class="separator1"></div>
<br />
<%--<asp:LinkButton ID="createBt" runat="server" CssClass="bt1" onclientclick="javascript: __doPostBack('Approve', ''); return false;"><span>ENDORSE &raquo;</span></asp:LinkButton>&nbsp;
...
<a href="cfo_VendorList.aspx" class="bt1"><span>BACK</span></a>--%>
<asp:LinkButton ID="createBt" runat="server" CssClass="bt1" onclientclick="javascript:if(confirm('Are you sure to Approve this Vendor?')) if($('#ContentPlaceHolder1_Comment').val()!=''){  __doPostBack('Approve', ''); }else{ alert('Comment must have a value.'); $('#ContentPlaceHolder1_Comment').focus(); } return false;"><span>APPROVE &raquo;</span></asp:LinkButton>&nbsp;
<asp:LinkButton ID="LinkButton1x" runat="server" CssClass="bt1" onclientclick="javascript:if(confirm('Are you sure to Disapprove this Vendor?')) if($('#ContentPlaceHolder1_Comment').val()!=''){  __doPostBack('Disapprove', ''); }else{ alert('Comment must have a value.'); $('#ContentPlaceHolder1_Comment').focus(); } return false;"><span>DISAPPROVE &raquo;</span></asp:LinkButton>&nbsp;
        <asp:LinkButton ID="createBt1" runat="server" CssClass="bt1" onclientclick="javascript:javascript:if(confirm('Are you sure to Clarify this report to VM Head?')) if($('#ContentPlaceHolder1_Comment').val()!=''){  __doPostBack('clarifyThis', ''); }else{ alert('Comment must have a value.'); $('#ContentPlaceHolder1_Comment').focus(); } return false;"><span>CLARIFY</span></asp:LinkButton>&nbsp;
<a href="pvmd_VendorList.aspx" class="bt1"><span>BACK</span></a>
<br />
<br />
<br />
    </form>
